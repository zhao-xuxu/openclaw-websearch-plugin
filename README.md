...
装了这个插件后，Agent 有了**专用搜索工具**，行为变得克制、精准。实测速度对比：
...
**Skill**：一个 SKILL.md 文件，告诉 Agent 如何通过 Bash 调用外部 Shell 脚本（.sh）。需要 bash、curl、jq 等系统命令，输入输出都是文本，Agent 需要自己解析。

**Plugin Tool**：通过 `api.registerTool()` 注册为原生工具，直接出现在 Agent 的工具列表中。Agent 按名称调用（如 `grok_search`），参数有 JSON Schema 校验，结果是结构化数据。
...
- **Skill → Plugin Tool** — 从 Shell 脚本改为 `api.registerTool()` 原生工具，Agent 直接调用，参数校验和错误处理更可靠
- **去掉 TavilyProxyManager 容器** — 原版通过单独的 Docker 容器做 Tavily 多 Key 代理；本项目将轮转逻辑内置于插件代码中（遇到 429/401 自动切换下一个 Key）
- **支持 Grok 中转站** — 原版只能通过本地 grok2api Docker 访问 Grok；本项目额外支持任意 OpenAI 兼容的中转站，使用中转站时不需要任何 Docker
...
**Tavily 多 Key 轮转** — 使用粘性策略：一直用当前 Key，遇到 429（额度耗尽）或 401（失效）后自动切到下一个。多个免费 Key 可叠加额度。

**ws_fetch 降级链** — `Tavily Extract（批量，首选）→ FireCrawl Scrape（逐 URL，备选）→ 报错`。不配置 FireCrawl Key 则只有第一级。
...
| Grok 报错 | Docker：Token 不在 ssoBasic 池 | 重新添加到 ssoBasic |
| Grok 报错 | Docker：CF 过期 (403) | 重启容器触发刷新 |
| Tavily 401/429 | Key 无效或额度耗尽 | 会自动轮转；全部耗尽则需加 Key |
| 飞书/WeCom 回复中断 | LLM API 限流 | 等限流窗口过后重试 |
